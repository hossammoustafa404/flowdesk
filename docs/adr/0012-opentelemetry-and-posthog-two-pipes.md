...
We considered OpenTelemetry only (Insights would stay empty) and the PostHog SDK only (no Prisma waterfall). Observability is opt-in: missing config or a PostHog outage must not fail auth or Health.
