...
Auth product Events (`user signed up`, `user signed in`, `user signed out`) are captured from `AuthEventsHook` via `PosthogService`. Sign-up and sign-in use AfterHooks. Sign-up Organization creation runs in `SignUpOrganizationHook` before the events after-hook so a failed Organization create does not emit `user signed up`. Sign-out uses `@AfterDelete("session")` because `/sign-out` does not run `sessionMiddleware`, so `ctx.context.session` is null on HTTP hooks. The database hook receives the Session row (`userId`) and the endpoint context (Origin, path). Only `/sign-out` is captured so Session expiry on get-session and revoke are not Events. `PosthogService.capture()` swallows PostHog failures so they cannot 500 auth. Origin-gate stays policy-only BeforeHooks (including Sign-up body validation). Requires `hooks: {}` and `databaseHooks: {}` in `createAuth`.

## Email verification and queued mail

Owner Sign-up requires Email verification (`requireEmailVerification: true`). Sign-up creates the User, Organization, and Owner Membership together, then enqueues a plain-text verification email. It does not issue a Session. The Owner signs in after calling Better Auth’s verify URL. A second sign-up with the same email returns `409 CONFLICT` with `USER_ALREADY_EXISTS_USE_ANOTHER_EMAIL` from the origin gate. Better Auth would otherwise return a synthetic 200 to hide whether the email is registered.

Auth owns copy. `createAuth` receives the mail queue and enqueues `{ to, subject, text, html }` from `sendVerificationEmail` (plain text, `html` empty). Enqueue failure is logged and sign-up still succeeds once User + Organization + Owner are durable: Better Auth persists the User first, Organization creation runs in the Sign-up after-hook (rolling back the User if Organization create fails), then Better Auth swallows email-callback errors in `runInBackgroundOrAwait`, so throwing from mail cannot fail the request or roll back durable rows. A second sign-up with the same email is `409`. Better Auth signs a JWT and verifies it — it does not persist a verification row for this flow. BullMQ’s Redis connection lives in `shared/queue/`. Shared mail lives in `shared/mail/`: the mail queue, Resend, and the processor in the same Nest process. Resend failure is retried by the queue, except reserved test recipients (`example.com`) which are skipped when `NODE_ENV` is not `production`, and other 4xx responses (except 408, 425, and 429) which fail without retry. Network errors from Resend bubble and retry.
...
Public Sign-up is the only self-serve Organization create path: the shared Sign-up contract requires Organization name; slug is derived server-side; `SignUpOrganizationHook` creates the Organization and Owner Membership after the User row (and deletes the User if that create fails). `allowUserToCreateOrganization` is `false` so org Users cannot self-serve create another Organization after Sign-up. Creator Membership position remains owner for any allowed create path (e.g. future Super Admin support). Session create sets Active Organization to the User’s Membership Organization (Sign-up verification and later sign-in). Teams and dynamic Organization roles stay off. Prisma has Organization, Membership (`member`), Invitation, and `activeOrganizationId` on Session — no team, teamMember, or organizationRole tables. Plugin access control enforces owner/admin/member on plugin routes. Do not introduce CASL for Organization this pass.
