...
- Validate env with a Zod schema colocated in `shared/config/env.schema.ts`. **NEVER** name it `env.ts`. **NEVER** put that schema in `@flowdesk/schemas`. **NEVER** add `env.schema.spec.ts`. See `validation.md`, `testing.md`.
