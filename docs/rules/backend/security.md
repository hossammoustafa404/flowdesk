...
- Never trust client input. Validate HTTP payloads with Zod via `@flowdesk/schemas`. See `validation.md`.
- Store secrets in server env only (`BETTER_AUTH_SECRET`, `DATABASE_URL`, OAuth keys, `RESEND_API_KEY`). **NEVER** expose them to clients.
- Validate server env in `apps/server/src/shared/config/`. **NEVER** put secret or env schemas in `@flowdesk/schemas` — `web` and `admin` could import them.
